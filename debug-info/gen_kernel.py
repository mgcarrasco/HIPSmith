...
def header_search_path(binary):
    """Where to look for the required headers, most specific first.

    safe_math_macros.h is generated by m4 at build time so it only exists in a
    build directory; HIPSmithPrint.h may be missing from a build directory that
    was configured before it was added, hence the runtime/ fallback.
    """
    dirs = [binary.parent, REPO / "runtime"]
    dirs += [REPO / name for name in BUILD_DIRS]
    return dirs
...
def main():
    ...
    search = header_search_path(binary)
    for header in REQUIRED_HEADERS:
        for directory in search:
            ...
            sys.exit(f"error: {header} not found in "
